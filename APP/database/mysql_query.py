...
class MysqlQuery(Enum):
    ...
    REGISTER_USER = 'insert into users (login, password, email, faculty_id, status) values("{}","{}", "{}", {}, "{}")'
    LOGIN_USER = 'select id from users where login="{}" and password="{}" and status = "{}"'
    GET_USER_ID = 'select id from users where login="{}" and password="{}" and email="{}"'
